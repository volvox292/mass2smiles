...
outfile = os.path.join(path_data, 'nist_cache_pos.pickle')
...
for count,i in enumerate(spectrums):
    print(count)
    if i.metadata['inchikey'] not in modified_spectra:
        modified_spectra[i.metadata['inchikey']]=i
    else:
        first_mz_intensity_dict=dict(zip(modified_spectra[i.metadata['inchikey']].peaks.mz, modified_spectra[i.metadata['inchikey']].peaks.intensities))
        ...
        modified_spectra[i.metadata['inchikey']].peaks=Spikes(mz=np.array(list(update_sorted.keys()), dtype="float"), intensities=np.array(list(update_sorted.values()), dtype="float"))
        
mod_specs_matchms=list(modified_spectra.values())

pickle.dump(mod_specs_matchms, 
            open(os.path.join(path_data,'composite_nist_pos_math_500.pickle'), "wb"))
